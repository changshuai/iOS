        ...
        progress:(void (^)(NSInteger, NSInteger, CGFloat))progressBlock state:(void (^)(DownloadState))stateBlock
{
    ...
    // 创建流
    NSOutputStream *stream = [NSOutputStream outputStreamToFileAtPath:localFile append:YES];
    ...
    // 设置请求头
    NSString *range = [NSString stringWithFormat:@"bytes=%zd-", downloaded_size];
...
{
    [self download:url local:HSFileFullpath(url) size:HSDownloadLength(url) progress:progressBlock state:stateBlock];
}
...
{
    NSMutableArray* array = [[DownloadDao sharedManager] getDownloadObject:down_obj.url];
    DownloadObject* newObj = [[DownloadObject alloc] init];
    
    if ([array count] > 0) {
        
        newObj = [array objectAtIndex:0];

    } else {
        
        newObj.url = down_obj.url;
        newObj.localPath =HSFileFullpath(down_obj.url);
        newObj.currentSize = 0;
        newObj.totalSize = 0;
        newObj.status = 0;
        newObj.className =[down_obj className];
        newObj.objectJson = [(NSObject*)down_obj JSONString];
        
        [[DownloadDao sharedManager] insertDownloadObject:newObj];
    }
    
    [self download:down_obj.url local:HSFileFullpath(down_obj.url) size:newObj.currentSize progress:progressBlock state:stateBlock];
}
...
- (void)start:(NSString *)url
{
    ...
    [self updateDownloadObjectStatus:url status:DownloadStateStart];
...
- (void)pause:(NSString *)url
{
    ...
    [self updateDownloadObjectStatus:url status:DownloadStateSuspended];
...
{
    NSMutableArray* array = [[DownloadDao sharedManager] getDownloadObject: url];
    
    if ([array count] > 0) {
        DownloadObject* obj = [array objectAtIndex:0];
        if (obj.currentSize == obj.totalSize && obj.currentSize != 0){
            return YES;
        }
    }
    return NO;
...
- (CGFloat)progress:(NSString *)url
{
    ...
    if ([array count] > 0) {
        DownloadObject* obj = [array objectAtIndex:0];
        return obj.totalSize == 0 ? 0 : 1.0 * obj.currentSize/obj.totalSize;
    }
    return 0.0;
}
//
...
- (void)deleteFile:(NSString *)url
{
    ...
    if ([fileManager fileExistsAtPath:HSFileFullpath(url)]) {
        ...
        [[DownloadDao sharedManager] deleteObj:url];
    }
}
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    NSMutableArray* array = [[DownloadDao sharedManager] getDownloadObject:sessionModel.url];
    if ([array count] > 0) {
        DownloadObject* obj = [array objectAtIndex:0];
        obj.totalSize = totalLength;
        obj.status = DownloadStateStart;
        [[DownloadDao sharedManager] updateDownloadObject:obj];
    }
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error
{
    ...
    if ([array count] > 0) {
        DownloadObject* obj = [array objectAtIndex:0];
        if (obj.currentSize == obj.totalSize ){
            obj.status = DownloadStateCompleted;
            sessionModel.stateBlock(DownloadStateCompleted);
        }
        else if (error){
            obj.status = DownloadStateFailed;
...
-(id) getDonwloadObject:(NSString*) url {
    NSMutableArray* array = [[DownloadDao sharedManager] getDownloadObject:url];
    if ([array count] > 0) {
        DownloadObject* obj = [array objectAtIndex:0];
        return [DownloadObject getClassObject:obj];
    }
    return nil;
}

-(NSArray*) getDownloadObjectList:(DownloadState) status {
    NSArray* array = [[DownloadDao sharedManager] getDownloadObjectList:status];
    NSMutableArray* mutableArray = [[NSMutableArray alloc] init];
    for (DownloadObject* obj in array) {
        id newObject = [DownloadObject getClassObject:obj];
        [mutableArray addObject:newObject];
    }
    return mutableArray;
}

-(void) updateDownloadObjectStatus:(NSString*) url status:(DownloadState) status {
    NSMutableArray* array = [[DownloadDao sharedManager] getDownloadObject:url];
    if ([array count] > 0) {
        DownloadObject* obj = [array objectAtIndex:0];
        obj.status = status;
