...
{
    CGFloat percent = [[HSDownloadManager sharedInstance] progress:downloadUrl1] * 100;
...
{
    TestObject* test = [TestObject getInstance];
    DownloadObject* obj = [[DownloadObject alloc] init];
    obj.url = url;
    obj.className = @"testObject";
    obj.objectJson = [test JSONString];
    
    [[HSDownloadManager sharedInstance] downloadObject:obj progress:^(NSInteger receivedSize, NSInteger expectedSize, CGFloat progress) {
